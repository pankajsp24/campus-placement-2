...
if(isset($_POST['submit'])){
   ...
   if($_POST["newPassword"] !== $_POST["confirmPassword"]){
      echo" <script>alert('New password and confirmed password must be same!')
          ...
          //fetch pass
         $result = mysqli_query($connection,"SELECT * FROM students WHERE PRN ='" . $PRN . "'") or die(mysqli_error($connection));
         $row = mysqli_fetch_array($result);

            if($_POST["currentPassword"] == $row["password"]){
              //update password
              $result = mysqli_query($connection,"UPDATE students SET password = '" . $_POST["newPassword"] . "' WHERE PRN ='" . $PRN . "'");
...
<div class="megacontainer" style="padding-top: 0px">
  ...
  <div class="container">
    ...
    <input type="Password" name="confirmPassword" placeholder=" Re-enter Password" required="">
      ...
      <center><h2><b><a href="student_dashboard.php">Back</a></b></h2></center>
